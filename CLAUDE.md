...
- **品名一致化**：各店貼文各自打字，同一商品有大量寫法差異（價格尾綴 `850元`／全半形括號／
  ...
  是「型號 → 標準品名」的單一來源，`draw-build` 產 data.js 時套用；**單顆陀螺的標準品名尾巴
  接原裝配置**（`BX-26 獨角刺心 5-60GP`、CX 前綴輔助刃 `CX-13 龍王閃擊 K1-50I`，來源 products.json
  的 ratchet／bit／assist；隨機強化組、改造組、對戰組這種多顆的不接）——用戶 2026-09-03 決定，
  方便在清單上直接看固鎖與軸心；**正本 `source-links.txt`
  一律保留各店原文**（那是抄自貼文的證據，不要為了統一去改它）。唯一例外是**型號打錯**——
  它會讓該筆自成一顆錯的篩選籤（2026-09-03 新店誠品 BXG-01 被上游寫成 BX-00），這種直接在正本改對，
  sync 對同批既有連結沿用正本品名、不會覆寫。型號抽取 `tagOf` 的唯一來源在
...
- 店家排序＝到 `anchors.tsv` 最近錨點的直線距離＋`stores.tsv` 第 6 欄體感調整；
  **距離與錨點只在 build 階段使用，不寫進 `data.js`、畫面也不顯示**（那是使用者的活動範圍）。
  縣市內要照指定先後時用 `store_order.tsv`（用戶 2026-09-11 指定台北市順序）：只換該縣市的店彼此的先後，
  它們在全站清單裡佔的位置不動（跨縣市穿插照距離）；`scripts/draw-stores.mjs` 的 `applyCityOrder`，對不到店名就 throw
...
- **⚡ 全力抽選模式**（`#focusBtn`，`aria-pressed`；用戶 2026-09-11）：開啟時閃電塗黃，清單**不畫已抽的品項**，
  ...
  模式不能丟），**不進網址也不進條件記憶**（個人進度，分享出去不該藏別人的清單）。剛點的那筆先灰成「已抽」，
  頁面被蓋住（`visibilitychange` hidden）就立刻收掉，否則 2 秒後由計時器收（不靠回到這頁的事件）；
  同分頁真的跳走時 `beforeunload` 把計時器延到 10 秒（舊頁還看得見時收掉那列會害人點錯下一筆），理由見 lessons L15。
  回前景照 localStorage 重畫，但**讀不到 storage 時沿用記憶體**、不洗掉已抽。
  說明彈窗的兩個步驟下方有一段它的說明
...
- **已抽清除跟著篩選走**：開著篩選時那顆橘鈕變 `已抽 v/n · 清除顯示中`、confirm 也講明範圍，
  只清目前看得到的；沒篩選＝全清（跟原本一樣）。範圍必須寫在字面上，
  不然使用者以為清了全部
- **條件記憶（`funbox:presets:v1`，至多 3 組）**：🔍 面板頂的紫色 chips，把目前複選
  （縣市/店家/品項/搜尋）存成一組、一鍵套回；存的時候跳原生 prompt 可**選擇性命名**
  （預填自動標籤、直接確定＝用自動的、取消＝不存、上限 20 字；自訂名存進 `name` 欄）。
  存 localStorage——單機便利性，分享仍走網址參數。同組合不重複存、滿三組收起「存目前」。
  GA 的 `filter_use` 多一種 `filter_kind=preset`
- **待公布的店照樣列在縣市／店家篩選裡**（數量欄寫「待公布」，只有整修中的不列）：換批期間
  大部分店沒品項，只列有品項的店就沒辦法先把常去的店存成條件（用戶 2026-09-10）。
  選的店全都待公布時，清單空白處寫「還沒公布」而不是叫人換關鍵字。
  清單底部「整修中，這批沒有抽選：…」那行只在**選了該店所在縣市**時才出現（沒選縣市不寫）
